...
export default class Toast {
    ...
    constructor(options, $referrer = null) {
        ...
        const $item = $('<li/>', { class: 'toast-item' });
        const $toast = $('<div/>', { class: 'toast background-' + this.options.style });
        const $icon = $('body').find('svg.icon').eq(0).clone();
        const $title = $('<div/>', { class: 'toast-title' });
        const $text = $('<p/>', { class: 'toast-text' });
        ...
        if (this.timeout) {
            setTimeout(() => { $item.remove() }, this.options.timeout);
        }
